...
class DataStore {
    ...
    async getSensorHistory(options = {}) {
        const { page = 1, limit = 10, sortBy = 'timestamp', sortOrder = 'desc', search = '' } = options;

        let whereClause = {};

        if (search) {
            const numSearch = !isNaN(parseFloat(search)) ? parseFloat(search) : undefined;
    ...
    async getActivityLogs(options = {}) {
        const { page = 1, limit = 20, device = '', action = '', status = '', search = '', sortOrder = 'desc' } = options;
