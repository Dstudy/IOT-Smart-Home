...
class MqttService extends EventEmitter {
    ...
    async handleSensorData(payloadStr) {
        ...
        if (data.temp !== undefined && data.hum !== undefined && data.lightValue !== undefined) {
            const sensorData = {
    ...
    async toggleDevice(deviceName, targetState) {
        return new Promise((resolve, reject) => {
            ...
            const command = targetState === 'ON' ? `On${espDeviceName}` : `Off${espDeviceName}`;

            // Create a listener for the response
            const onResponse = (response) => {
                // Ensure the response matches our device and action
                if (response.device.toUpperCase() === espDeviceName.toUpperCase() && response.action === targetState) {
