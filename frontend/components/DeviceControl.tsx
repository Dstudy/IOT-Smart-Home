...
const DeviceControl: React.FC<DeviceControlProps> = ({
    ...
    const getColorClass = () => {
        ...
        if (isOn) {
            if (displayName === 'Air Conditioner') return 'bg-blue-500 hover:bg-blue-600 text-white';
            if (displayName === 'Fan') return 'bg-teal-500 hover:bg-teal-600 text-white';
            if (displayName === 'Light') return 'bg-yellow-400 hover:bg-yellow-500 text-white';
            ...
            >
                {isLoading ? (
                    ...
                    <span className="flex items-center space-x-2">
                        <span className={`w-2.5 h-2.5 rounded-full ${isOn ? 'bg-white' : 'bg-gray-400'} shadow-sm`}></span>
